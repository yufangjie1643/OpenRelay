...
const trayIconPath = path.join(root, '..', 'assets', 'litellm.ico');
...
assert(!/localhost:4000|port `4000`|--port', '4000'|--port 4000/.test(serverJs), 'server must not depend on a separate LiteLLM service on port 4000');
...
assert(fs.existsSync(trayIconPath) && fs.statSync(trayIconPath).size > 0, 'LiteLLM tray icon should be stored at assets/litellm.ico');
assert(fs.existsSync(webTrayIconPath) && fs.readFileSync(webTrayIconPath).equals(fs.readFileSync(trayIconPath)), 'web/icon.ico should match the LiteLLM tray launcher icon');
assert(/IDI_APP_ICON\s+ICON\s+"assets\/litellm\.ico"/.test(trayResource), 'start-tray.rc should embed assets/litellm.ico as the application icon');
assert(!/localhost:4000/.test(indexHtml), 'admin UI should advertise only the unified 18783 endpoint');
assert(!/4000|litellm\.exe|\.venv/.test(startBat + startAllBat + setupPs1), 'startup/setup scripts must not start a separate 4000 LiteLLM service');
