...
This repository contains a Windows-oriented unified Node.js LiteLLM-compatible proxy plus a small admin UI.

- `web/server.js` hosts the Express app, manages config migration, writes `litellm-config.yaml`, tracks usage, and serves the unified OpenAI-compatible proxy on port `18783`.
...
- Runtime files such as `config.json`, `litellm-config.yaml`, `usage.jsonl`, and `conversations/` are local/generated and should not be committed.
...
- `.\start.bat` starts the unified Web UI / API at `http://localhost:18783`.
- `.\start-all.bat` opens the unified Web UI / API in a Windows command window.
